...
namespace entity
{
    ...
    {
    public:
        MovementComp(Player& player) : _player{player}
        {
        }
        ...
        void Tick()
        {
            ...
            // struct Block
            //     block = BLOCKS[world_get_block(entity.ecs->world, c_position->block)],
            //     top_block = BLOCKS[world_get_block(
            //         entity.ecs->world,
            //         world_pos_to_block(glms_vec3_add(
            //             c_position->position, (vec3s){{0.0f, c_physics->size[1].y, 0.0f}})))];

            // const f32 speed = 0.0245f * c_movement->speed *
            ...
            // forward =
            //     (vec3s){{sinf(c_camera->camera.yaw), 0, cosf(c_camera->camera.yaw)}};
            ...
            //                    exiting = breaching &&
            //             (c_physics->stopped.x || c_physics->stopped.z);
            //         const f32 float_speed = speed * 0.7f *
            //             (breaching ? 1.4f : 1.0f) * (exiting ? 2.0f : 1.0f);
